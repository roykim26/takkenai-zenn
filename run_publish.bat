...
set "LOG_FILE=%LOG_DIR%\schedule.log"
...
>> "%LOG_FILE%" echo.
>> "%LOG_FILE%" echo ===== publish run start %RUN_START% =====
...
>> "%LOG_FILE%" echo main.py output:
"%PYTHON_EXE%" main.py >> "%LOG_FILE%" 2>&1
set "EXIT_CODE=%errorlevel%"

>> "%LOG_FILE%" echo main.py exit code=%EXIT_CODE%
...
>> "%LOG_FILE%" echo ===== publish run end %RUN_END% exit=%EXIT_CODE% =====
